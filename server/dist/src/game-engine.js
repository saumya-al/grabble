...
class GrabbleEngine {
    ...
    /**
     * Remove tiles from player's rack by indices
     * Used when placing tiles on the board
     */
    removeTilesFromRack(playerId, tileIndices) {
        const player = this.state.players.find(p => p.id === playerId);
        if (!player) {
            throw new Error(`Player ${playerId} not found`);
        }
        console.log(`🔧 removeTilesFromRack called for player ${playerId}, rack before:`, player.rack.map(t => t.letter).join(','), 'indices:', tileIndices);
        ...
        for (const idx of sortedIndices) {
            if (idx >= 0 && idx < player.rack.length) {
                ...
                console.warn(`🔧 Invalid index ${idx} for rack length ${player.rack.length}`);
            }
        }
        console.log(`🔧 removeTilesFromRack completed, rack after:`, player.rack.map(t => t.letter).join(','));
    }
    /**
